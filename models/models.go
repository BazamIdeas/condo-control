...
func init() {
	...
	orm.RegisterModel(new(Assistances), new(Condos), new(Holidays), new(Points), new(Supervisors), new(Verifications), new(Watchers), new(Workers), new(Zones), new(Tasks), new(Goals), new(GoalsComments), new(Deliveries), new(Items), new(Notifications), new(Objects), new(Occurrences), new(Checks))
